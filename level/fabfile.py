...
_TAR_FILE = 'dist-level.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/level'
...
def build():
    ...
    with lcd(os.path.join(_current_path(), 'learnwebbypython')):
...
def deploy():
    newdir = 'learnwebbypython-%s' % _now()
    ...
    with cd(_REMOTE_BASE_DIR):
        sudo('rm -f learnwebbypython')
        sudo('ln -s %s learnwebbypython' % newdir)
        sudo('chown root:root learnwebbypython')
        sudo('chown -R root:root %s' % newdir)
    with settings(warn_only=True):
        sudo('supervisorctl stop level')
        sudo('supervisorctl start level')
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        r = run('ls -p -1')
        files = [s[:-1] for s in RE_FILES.split(r) if s.startswith('learnwebbypython-') and s.endswith('/')]
        files.sort(cmp=lambda s1, s2: 1 if s1 < s2 else -1)
        r = run('ls -l learnwebbypython')
        ss = r.split(' -> ')
        if len(ss) != 2:
            print ('ERROR: \'learnwebbypython\' is not a symbol link.')
            return
        current = ss[1]
        ...
        sudo('rm -f learnwebbypython')
        sudo('ln -s %s learnwebbypython' % old)
        sudo('chown root:root learnwebbypython')
        with settings(warn_only=True):
            sudo('supervisorctl stop level')
            sudo('supervisorctl start level')
