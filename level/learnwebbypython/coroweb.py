...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        ...
        if found and(param.kind != inspect.Parameter.VAR_POSITIONAL and param.kind != inspect.Parameter.KEYWORD_ONLY and param.kind != inspect.Parameter.VAR_KEYWORD):
           raise ValueError('request parameter must be the last named parameter in function %s parameters:%s' % (fn.__name__, str(sig)))
    return found
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            if request.method == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    kw = params
                elif ct.startswith('application/x-www-form-urlencoded')or ct.startswith('mutlipart/form-data'):
